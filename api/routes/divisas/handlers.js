...
const findOne = (req, res) => {
  Divisa.findOne({ _id: req.params.id }, (err, query_response) => {
    if (err) {
...
const uncreate = (req, res) => {
  Divisa.deleteOne({ _id: req.params.id }, err => {
    if (err) {
      res.status(404).json({ error: 'La Divisa no se puede eliminar.' });
    } else {
...
const update = (req, res) => {
  Divisa.updateOne({ _id: req.params.id }, { $set: req.body }, err => {
    if (err) {
      res.status(404).json({ error: 'La Divisa no se puede modificar.' });
    } else {
